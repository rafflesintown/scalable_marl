...
if __name__ == "__main__":
    ...
    ### parameter that
    parser.add_argument(
        "--alg", default="sac_v2", help="The algorithm to use. rfsac or sac."
    )
    parser.add_argument(
        "--env",
        default="custom_vec",
    ...
    )  # rf sigma (actually not useful for this alg)
    ...
    if args.alg == "sac_v2":
        ...
        agent = rfsac_agent_network.CustomModelRFSACAgent(
            dynamics_fn=dynamics, rewards_fn=rewards, **kwargs
        )
    ...
    for t in range(int(args.max_timesteps + args.start_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            _, V_ret, V_mean_err = util_network.batch_eval_discounted(
                agent, eval_env, return_features=False
            )
