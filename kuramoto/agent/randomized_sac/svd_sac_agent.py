...
class svdSACAgent(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.nn_mu_net = learn_phi.nnMu(
            state_dim, output_dim=self.rsvd_num, hidden_dim=256
        ).to(self.device)
        ...
        self.mu_optimizer = torch.optim.Adam(
            self.nn_mu_net.parameters(), lr=1e-4, betas=[0.9, 0.999]
        )
    ...
    def feature_step(self, batch, batch2):
        ...
        info = {"loss_contrast": loss_contrast.item(), "loss_self": loss_self.item()}
        return info
