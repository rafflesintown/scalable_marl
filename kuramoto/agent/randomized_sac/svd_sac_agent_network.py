...
class CustomPhiSACAgent(SACAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.mu_nets = [
            nnMu(eval_kappa_minus_one_obs_dim, output_dim=rsvd_num).to(self.device)
            for _ in range(self.N)
        ]
        ...
        self.mu_optimizer = torch.optim.Adam(
            all_mu_params, lr=self.lr, betas=[0.9, 0.999]
        )
    ...
    def feature_step(self, batch, batch2):
        ...
        shuffled_next_concat = local_states_concat2[
            torch.randperm(local_states_concat2.size(0))
        ]
        ...
        new_mu_optimizer = torch.optim.Adam(
            [mu_params[key] for key in mu_params_need_grad_keys],
            lr=self.lr,
            betas=[0.9, 0.999],
        )
        ...
        mu_self = vmap(self.fmodel_mu, in_dims=(0, 0, 1), out_dims=1)(
            mu_params, mu_buffers, local_next_states_concat
        )
        mu_contrast = vmap(self.fmodel_mu, in_dims=(0, 0, 1), out_dims=1)(
            mu_params, mu_buffers, shuffled_next_concat
        )
